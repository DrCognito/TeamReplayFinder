...
def build_metadata(match_in) -> dict:
    ...
    try:
        player_picks = match_in['players']
    except KeyError:
        print("Missing players_minimal")
        player_picks = {}
    for pp in player_picks:
        hero = dict(heroentity_template)
        steam_id = convert_to_64_bit(pp['account_id'])
        pickban_template['steamID'].append(steam_id)
        hero['steamid'] = steam_id

        hero_name = heroByID[pp['hero_id']]
...
def save_match_draft_odota(match_in, output_dir: Path = Path(environment['DRAFT_JSON_PATH'])) -> Path:
    try:
        ...
        print("Could not build a replay skeleton from odota.")
        return None
